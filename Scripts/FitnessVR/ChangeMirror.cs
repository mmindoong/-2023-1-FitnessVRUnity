...
namespace Mediapipe.Unity
{
  public class ChangeMirror : MonoBehaviour
  {
    ...
    bool isStart = false;
    public void ClearMirrorVisibility()
    {
      //CameraRotator.transform.rotation = Quaternion.Euler(0, 0, 0);
      MirrorFront.gameObject.SetActive(false);
      MirrorRight.gameObject.SetActive(false);
      MirrorTop.gameObject.SetActive(false);

      Camera cam1 = MirrorFront.GetComponent<Camera>();
      cam1.rect = new UnityEngine.Rect(0, 0, 1.0f, 1.0f);

      Camera cam2 = MirrorRight.GetComponent<Camera>();
      cam2.rect = new UnityEngine.Rect(0, 0, 1.0f, 1.0f);

      Camera cam3 = MirrorTop.GetComponent<Camera>();
      cam3.rect = new UnityEngine.Rect(0, 0, 1.0f, 1.0f);
    ...
    void Update()
    {
      ...
      if (userController.IsStarted || kinectController.IsStarted)
      {
        isStart = true;
        StartCoroutine(CallFunctionRepeatedly(3.0f));
      }
    }

    IEnumerator CallFunctionRepeatedly(float interval)
    {
      while (isStart)
      {
        CallDecisionMirrorView();

        yield return new WaitForSeconds(interval);
      }
    }

    void CallDecisionMirrorView()
    {
      // 판정하는 2차원 좌표축
      ...
      if (isStart)
      {
        ...
        if (FirstMax == FrontXYCount)
        {
          ...
          if (SecondMax == XYcnt)
          {
            ClearMirrorVisibility();
            MirrorFront.gameObject.SetActive(true);
            View.text = "Mirror Front";
          }
          else
          {
            ...
            else
            {
              ...
              if (SecondMax == ZYCnt)
              {
                ...
                Camera cam = MirrorRight.GetComponent<Camera>();
                cam.rect = new UnityEngine.Rect(0, 0, 0.5f, 1.0f);
                View.text = "Mirror Front&Side";

              }
              else if (SecondMax == XZCnt)
              {
                MirrorFront.gameObject.SetActive(true);
                MirrorTop.gameObject.SetActive(true);
                Camera cam = MirrorTop.GetComponent<Camera>();
                cam.rect = new UnityEngine.Rect(0, 0, 0.5f, 1.0f);
                View.text = "Mirror Front&Top";
          ...
          else
          {
            ...
            else
            {
              ...
              if (SecondMax == XYcnt)
              {
                MirrorFront.gameObject.SetActive(true);
                MirrorRight.gameObject.SetActive(true);
                Camera cam = MirrorFront.GetComponent<Camera>();
                cam.rect = new UnityEngine.Rect(0, 0, 0.5f, 1.0f);
                View.text = "Mirror Side&Front";
        ...
        else if (FirstMax == TopXZCount)
        {
          ...
          else
          {
            if (FirstMax >= SecondMax)
            {
              ...
              View.text = "Mirror Top(움직임)";

            }
            else
            {
              ClearMirrorVisibility();
              Debug.Log("움직인 총 거리는 측면, 간격은 다른 뷰가 유리");
              /*
              // 멀티뷰
              if (SecondMax == XYcnt)
              {
                ...
                Camera cam = MirrorFront.GetComponent<Camera>();
                cam.rect = new UnityEngine.Rect(0, 0, 0.5f, 1.0f);
                View.text = "Mirror Top&Front";

              }
              else if (SecondMax == ZYCnt)
              {
                MirrorTop.gameObject.SetActive(true);
                MirrorRight.gameObject.SetActive(true);
                Camera cam = MirrorRight.GetComponent<Camera>();
                cam.rect = new UnityEngine.Rect(0, 0, 0.5f, 1.0f);
                View.text = "Mirror Top&Side";
        ...
        /*
        // 2. Count 가 동일하면 멀티뷰
        int XY = FrontXYCount + XYcnt;
        int YZ = SideZYCount + ZYCnt;
        int XZ = TopXZCount + XZCnt;
        if (XY == YZ)
        {
          ClearMirrorVisibility();
          MirrorFront.gameObject.SetActive(true);
