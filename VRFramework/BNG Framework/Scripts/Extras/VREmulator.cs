...
namespace BNG {
    ...
    public Mediapipe.Unity.ChangeMirror Mirror;
    public Text UI;
    private float speed;
      ...
      public InputActionReference SideMirrorAction;
    public InputActionReference FrontMirrorAction;
    public InputActionReference TopMirrorAction;

    bool IsSideRotating = false;
    bool IsTopRotating = false;
    bool IsFrontRotating = false;
    ...
    void Update() {
      ...
      if (IsSideRotating && PrevRotation == 0) // Front to Side
      {
        speed = 200.0f;
        float rotationThisFrame = speed * Time.deltaTime;

        if (cumulativeRotation < 90.0f)
        {
          Mirror.CameraRotator.transform.Rotate(0, rotationThisFrame, 0);
          cumulativeRotation += rotationThisFrame;
        }
        else
        {
          IsSideRotating = false;
          cumulativeRotation = 0.0f;
          Mirror.CameraRotator.transform.rotation = Quaternion.Euler(0, 90.0f, 0);
        }
      }
      else if (IsSideRotating && PrevRotation == 1) // Top->Front to Side
      {
        speed = 200.0f;
        float rotationThisFrame = speed * Time.deltaTime;
      ...
      if (IsFrontRotating && PrevRotation == 1) // Side to Front
      {
        speed = 200.0f;
        float rotationThisFrame = -speed * Time.deltaTime;
        ...
        {
          IsFrontRotating = false;
          cumulativeRotation = 0.0f;
          Mirror.CameraRotator.transform.rotation = Quaternion.Euler(0, 0, 0);
        }

      }
      else if (IsFrontRotating && PrevRotation == 2) // Top to Front
      {
        speed = 200.0f;
        float rotationThisFrame = speed * Time.deltaTime;
        ...
        if (cumulativeRotation <= 90.0f)
        {
          Mirror.CameraRotator.transform.Rotate(rotationThisFrame, 0, 0);
          cumulativeRotation += rotationThisFrame;
        }
        else
        {
      ...
      if (IsTopRotating && PrevRotation == 0) // Front to Top
      {
        speed = 200.0f;
        float rotationThisFrame = -speed * Time.deltaTime;
        ...
        else
        {
          IsTopRotating = false;
          cumulativeRotation = 0.0f;
          Mirror.CameraRotator.transform.rotation = Quaternion.Euler(-90, 0, 0);
        }

      }
      else if (IsTopRotating && PrevRotation == 1) // Side->Front to Top
      {
        speed = 200.0f;
        float rotationThisFrame = -speed * Time.deltaTime;
      ...
      if (SideMirrorAction.action.triggered) //to Side 회전
            {
                UI.text = "Mirror Side";
                IsSideRotating = true;
              if (Mirror.CameraRotator.transform.rotation == Quaternion.Euler(0, 0, 0)) //정면일때
                PrevRotation = 0; // PrevRotation 0:정면, 1:측면, 2:항공
              else if (Mirror.CameraRotator.transform.rotation == Quaternion.Euler(0, 90, 0)) // 측면일때
                PrevRotation = 1;
              else if (Mirror.CameraRotator.transform.rotation == Quaternion.Euler(-90, 0, 0)) //항공
              {
                PrevRotation = 2;
                Mirror.CameraRotator.transform.rotation = Quaternion.Euler(0, 0, 0);
              }
      ...
      //FrontMirrorAction.action.triggered
      if (FrontMirrorAction.action.triggered) //to Front 회전
        {
          UI.text = "Mirror Front";
          IsFrontRotating = true;
          if (Mirror.CameraRotator.transform.rotation == Quaternion.Euler(0, 0, 0)) //정면일때
            PrevRotation = 0; // PrevRotation 0:정면, 1:측면, 2:항공
          else if (Mirror.CameraRotator.transform.rotation == Quaternion.Euler(0, 90, 0)) // 측면일때
            PrevRotation = 1;
      ...
      // TopMirrorAction.action.triggered
      if (TopMirrorAction.action.triggered) //to Top 회전
            {
              IsTopRotating = true;
              UI.text = "Mirror Top";
            if (Mirror.CameraRotator.transform.rotation == Quaternion.Euler(0, 0, 0)) //정면일때
